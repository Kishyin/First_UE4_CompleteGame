...
void ASpawnVolume::BeginPlay()
{
	...
	if (Actor_1 && Actor_2 && Actor_3 && Actor_4 && Actor_5 && Actor_6 && Actor_7 && Actor_8 && Actor_9 && Actor_10)
	{
		...
		SpawnArray.Add(Actor_8);
		SpawnArray.Add(Actor_9);
		SpawnArray.Add(Actor_10);
...
{
	//Using Kismeth Math library the contain this function
	//It is used to get a random point in a box
...
void ASpawnVolume::SpawnOurActor_Implementation(UClass* ToSpawn, const FVector& Location)
{
	if (ToSpawn)
	{
		...
		if (World)
		{
			//Setting the spawn of the pawn creature, in a certain location
			AActor* Actor = World->SpawnActor<AActor>(ToSpawn, Location, FRotator(0.f), SpawnParams);

			AEnemy* Enemy = Cast<AEnemy>(Actor);
			if (Enemy)
			{
				//We want to spawn even the controller for eache enemy//
